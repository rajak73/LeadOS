...
export function LeadFilters() {
  ...
  return (
    <div className="space-y-4 p-4 bg-bg-elevated border border-border rounded-xl" data-testid="lead-filters">
      {/* Search */}
      <div>
      ...
      {/* Status filter */}
      <div>
        <label className="text-xs text-text-tertiary block mb-2">Status</label>
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded text-xs border transition-colors ${
                (filters.status ?? []).includes(s)
                  ...
                  : 'bg-bg-base border-border text-text-secondary hover:border-border/80'
              }`}
            >
              {formatLeadStatus(s)}
            </button>
      ...
      {/* Source filter */}
      <div>
        <label className="text-xs text-text-tertiary block mb-2">Source</label>
        <div className="flex flex-wrap gap-1.5">
            ...
            {savedPresets.map((p) => (
              <div key={p.name} className="flex items-center gap-1">
                <button
                  ...
                  className="px-2 py-0.5 rounded text-xs border border-border bg-bg-base text-text-secondary hover:text-text-primary hover:border-border/80 transition-colors"
                >
                  {p.name}
                </button>
                <button
