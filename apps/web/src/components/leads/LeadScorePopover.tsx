...
export function LeadScorePopover({ leadId, open, onOpenChange }: LeadScorePopoverProps) {
  ...
  let scoreColorClass = 'text-text-secondary border-border bg-bg-elevated';
  if (score !== null) {
    if (score >= 70) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-lg bg-bg-elevated border border-border rounded-xl p-6 shadow-2xl focus:outline-none max-h-[85vh] overflow-y-auto">
          <Dialog.Title className="text-lg font-semibold text-text-primary mb-4 flex items-center justify-between">
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              {/* Score Header */}
              <div className="flex items-center gap-4 p-4 rounded-xl border border-border bg-bg-base">
              ...
              {factors.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {positives.length === 0 ? (
                      <p className="text-xs text-text-tertiary">None identified</p>
                    ) : (
              ...
              {history.length > 1 && (
                <div className="border-t border-border/50 pt-4 space-y-2">
                  <h4 className="text-xs font-semibold text-text-tertiary uppercase tracking-wider">Scoring History</h4>
                  <div className="space-y-2 max-h-28 overflow-y-auto pr-1">
                    {history.map((h) => (
                      <div key={h.id} className="flex justify-between items-center text-xs p-1.5 hover:bg-bg-base/40 rounded transition-colors">
                        <span className="text-text-secondary font-medium">Score: {h.score}</span>
                        <span className="text-text-tertiary">{formatRelativeTime(h.createdAt)}</span>
                      </div>
                    ))}
                  </div>
                </div>
