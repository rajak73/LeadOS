  ...
  return (
    <div className="space-y-4" data-testid="deal-metadata-form">
      ...
      <div>
        ...
        {isReadOnly ? (
          <p className="text-base font-semibold text-text-primary">{deal.title}</p>
        ) : (
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            onBlur={() => title !== deal.title && handleBlur('title', title)}
            className="w-full text-base font-semibold text-text-primary bg-transparent border-b border-transparent hover:border-border focus:border-primary-500 focus:outline-none pb-0.5 transition-colors"
      ...
      <div className="flex gap-6">
        <div>
          <label className="text-xs text-text-tertiary block mb-1">Value</label>
          {isReadOnly ? (
            <p className="text-sm text-text-primary">{formatCurrency(deal.value, deal.currency)}</p>
          ) : (
            <input
              type="number"
              ...
              className="w-32 text-sm text-text-primary bg-transparent border-b border-transparent hover:border-border focus:border-primary-500 focus:outline-none pb-0.5 transition-colors"
        ...
        <div>
          <label className="text-xs text-text-tertiary block mb-1">Expected close</label>
          {isReadOnly ? (
            <p className="text-sm text-text-primary">{deal.expectedCloseDate?.split('T')[0] ?? '—'}</p>
          ) : (
            <input
              type="date"
              ...
              className="text-sm text-text-primary bg-transparent border-b border-transparent hover:border-border focus:border-primary-500 focus:outline-none pb-0.5 transition-colors"
      ...
      <div className="grid grid-cols-2 gap-4 text-xs text-text-tertiary border-t border-border/50 pt-4">
        <div>
          ...
          <span className={`font-medium ${deal.status === 'WON' ? 'text-green-400' : deal.status === 'LOST' ? 'text-red-400' : 'text-text-secondary'}`}>
            {deal.status}
          </span>
        </div>
        <div>
          <span className="block mb-0.5">Currency</span>
          <span className="text-text-secondary">{deal.currency}</span>
        </div>
        {deal.closedAt && (
          <div>
            <span className="block mb-0.5">Closed</span>
            <span className="text-text-secondary">{deal.closedAt.split('T')[0]}</span>
          </div>
        )}
        <div>
          <span className="block mb-0.5">Created</span>
          <span className="text-text-secondary">{deal.createdAt.split('T')[0]}</span>
        </div>
      </div>
