...
export function ActivityItemRow({ item }: { item: Activity }) {
  ...
  return (
    <div className="flex gap-3 py-3 border-b border-border/40 last:border-0">
      <div className="w-6 h-6 rounded-full bg-bg-subtle flex items-center justify-center text-xs text-text-secondary shrink-0 mt-0.5">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-text-primary">{describeActivity(item)}</p>
